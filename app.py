...
def extract_embedding(image_bytes: bytes) -> np.ndarray:
    ...
    if len(face_locations) > 1:
        face_locations = [
            max(face_locations, key=lambda f: (f[2]-f[0]) * (f[1]-f[3]))
        ]
...
def enroll_face():
    ...
    if 'walletAddress' not in request.form:
        return jsonify({'error': 'No walletAddress provided'}), 400
    ...
    if len(image_bytes) < 5000:
        return jsonify({'error': 'Image too small — capture failed'}), 400
    ...
    for record in all_enrolled:
        if record["wallet"] == wallet_address:
            continue
...
def verify_face():
    ...
    record = db.face_enrollments.find_one({"wallet": wallet_address})

    if not record:
        return jsonify({
            'error': 'Wallet not enrolled. Complete face enrollment first.'
        }), 404
